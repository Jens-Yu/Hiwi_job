...
def main():  # pragma: no cover
    ...
    dual_arm = DualArm(link_lengths, joint_angles_a, joint_angles_b, goal_pos_a, goal_pos_b, basic_point, show_animation)
    ...
    while True:
        ...
        if state_a is WAIT_FOR_NEW_GOAL:
            if distance_a > 0.1 and not solution_found_a:
                joint_goal_angles_a, solution_found_a = inverse_kinematics(
                    link_lengths, joint_angles_a, goal_pos_a)
        ...
        if state_b is WAIT_FOR_NEW_GOAL:
            if distance_b > 0.1 and not solution_found_b:
                joint_goal_angles_b, solution_found_b = inverse_kinematics(
                    link_lengths, joint_angles_b, goal_pos_b)
