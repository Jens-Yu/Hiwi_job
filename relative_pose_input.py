...
def main():  # pragma: no cover
    ...
    joint_angles = np.array([np.pi/2, 0, 0, np.pi/2, 0, 0])
    initial_pos_a = [3, 0]  # Desired goal position [x, y]
    initial_pos_b = [5, 0]  # Desired goal position [x, y]
    show_animation = True

    dual_arm = DualArm(link_lengths, joint_angles[:3], joint_angles[3:], initial_pos_a, initial_pos_b, basic_point, a_b, show_animation)
    state = WAIT_FOR_NEW_GOAL
    solution_found = False

    while True:
        dual_arm.update_joints(joint_angles[:3], joint_angles[3:])
        end_effector_a = np.array(dual_arm.end_effector_a)
        end_effector_b = np.array(dual_arm.end_effector_b)
        goal_pos = np.array(a_b)
        errors, distance = distance_to_goal((end_effector_b - end_effector_a), goal_pos)
        # State machine to allow changing of goal before current goal has been reached
        if state is WAIT_FOR_NEW_GOAL:
            if distance > 0.01 and not solution_found:
...
def inverse_kinematics(link_lengths, joint_angles, goal_pos):
    ...
    for iteration in range(N_ITERATIONS):
        ...
        errors, distance = distance_to_goal((current_pos_b - current_pos_a), goal_pos)
        print(f"Iteration {iteration}: joint_angles = {joint_angles}, errors = {errors}, distance = {distance}")  # Add this line
        if distance < 0.01:
...
def distance_to_goal(current_pos, goal_pos):
    x_diff = goal_pos[0] - current_pos[0] - basic_point[0]
    y_diff = goal_pos[1] - current_pos[1] - basic_point[1]
